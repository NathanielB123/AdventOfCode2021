...
agdefc cfad bdgecfa gadfe aefgc ad ceadgb gbafce bfedg agd | dag ad fcage cgbafe""".split("\n"))))
...
for num, segs in numSegIndex.items():
    lenNumIndex[len(segs)].append(num)

def perLine(line):
    uniqueDigits = line[0]
    outputDigits = line[1]
    uniquePossibilities = [list(range(0,10)) for _ in range(0,10)]
    outputPossibilities = [list(range(0,10)) for _ in range(0,4)]
    wireSegPossibilities = dict(zip(SEGS, map (lambda _: SEGS, SEGS)))
    done = False
    counter = 0
    while not done:
        counter+=1
        ...
        for i in range(4):
            outputPossibilities[i] = list(filter(lambda each: helperFunc(list(map(lambda wire: wireSegPossibilities[wire], outputDigits[i])), set(numSegIndex[each])), outputPossibilities[i]))
        if all(map(lambda each: len(each) == 1, outputPossibilities)):
            done = True
        totalPossibilities = uniquePossibilities + outputPossibilities
        totalDigits = uniqueDigits + outputDigits
        for wire in wireSegPossibilities.keys():
            wireSegPossibilities[wire] = (list(filter(lambda seg:
        ...
        segs = remToUnique(segs)
        wireSegPossibilities=dict(zip(wires,segs))
    return list(map(lambda x: x[0], outputPossibilities))
            ...
            if count >= len(each):
                for i2 in range(len(x)):
                    if x[i2] != each:
                        newList = []
                        for each2 in x[i2]:
                            if not each2 in each:
                                newList.append(each2)
                            else:
                                done = False
                        x[i2] = newList
    return x
